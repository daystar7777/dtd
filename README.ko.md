...
/dtd approve                     계획 락-인
/dtd run                         실행
/dtd run --silent=4h             조용히 진행; 블로커는 보류하고 안전한 task 계속 (v0.2.0f)
```

실행 모드 (Run styles, v0.2.0f):

```text
/dtd run --decision permission   기본: 권한/주요 결정 시 묻기
/dtd run --decision auto         안전한 진행 최대화 (destructive/유료/외부경로는 여전히 confirm)
/dtd interactive                 결정 필요할 때 즉시 묻기
/dtd silent on --for 4h          interrupt 안 함; 보류된 블로커는 나중에 보여주기
```
...
/dtd help [topic]                계층 도움말 (≤25줄 개요, ≤50줄 토픽)
/dtd update check                최신 DTD 버전 확인 (v0.2.0d)
/dtd r2 readiness                v0.3 라이브 테스트 진입 게이트; 워커 호출 없음
```
...
> 실패 분류) 은 v0.2.1+ Runtime Resilience에서 사용 가능.

...
secrets touched, no live worker calls. It produces a YES/NO answer
to "can R2 start?" and a concise STOP checklist if NO.
...
but doesn't block other R2 sub-releases.
...
r2_0_readiness(state, config):
  ...
  for harness in [check-v020b, check-v020c, ..., check-v030e, smoke]:
    ...
    if not res.pass:
      return Stop("static contracts failing", remediate="fix harness failures first")

  # Step 2 — Local DTD install health.
  doctor = run_doctor()
  errors = [c for c in doctor.codes if c.severity == "ERROR"]
  if errors:
    return Stop(f"doctor errors: {errors}", remediate=doctor.suggestions)
  ...
  if consensus_capable < 3:
    return Stop(f"need >= 3 worker slots; have {consensus_capable}", remediate="add workers via /dtd workers add")

  with_headers = count(workers, where=quota_provider_header_prefix is not null)
  if with_headers < 1:
    return Stop("need >= 1 worker with quota_provider_header_prefix",
  ...
  if not reviewer_distinct:
    return Stop("no reviewer-distinct worker for L-C-2",
  ...
  sync_ok = false
  if config.session_sync.enabled:
    key_env_set = os.environ.get(config.session_sync.encryption_key_env) is not None
    sync_target_writable = check_writable(config.session_sync.sync_path or config.session_sync.sync_branch)
    if key_env_set and sync_target_writable:
      sync_ok = true
    else:
      report_warn("v0.3.0d L-D-* coverage will be SKIPPED (sync not ready)")
...
If R2-0 returned `Stop`, the user gets ONE of these checklists.
Each has a concrete remediation step + verification command.
...
Verify:
  ...
  /dtd workers test <id> --quick   # confirm reachability per slot
                                   # (optional; full health probe
                                   # is R2-2)
```
...
without:
  ...
  [REQUIRED if L-D-*] Either sync_path (filesystem backend) or
                     sync_branch (git_branch backend) writable.
...
Remediation (only if user wants L-D-* coverage):
  ...
  3. Verify writability: write + delete a test file at sync_path
     (filesystem) OR `git fetch <remote> <sync_branch>` (git_branch).
...
- r2_0_no_quota_header_worker (WARN — only when R2 is being initiated)
...
1. STOP cases produce concrete checklists with verification
   commands.
2. GO outputs a `r2_0_decision` row that R2-1 can consume.
3. SKIP-not-FAIL distinction for sync prerequisites is explicit
   in the decision row.
4. No live worker calls happen at R2-0 (mock + observational only).
